...
window.onload = function () {
  ...
  let faceType;
  //結果ページのURL
  let resultURL = "http://" + window.location.host + pathname + "/result.html";
  //4ページ目
  function fourthPage() {
    //p3非表示・p4表示
  ...
  async function api(FILE_URL) {
    ...
    function Keisoku(mrks) {
      ...
      /*Q1-----------------------------------------------------------------*/
      //両頬の頬骨間の長さX：15番Xー1番X
      let hooboneX = (mrks[15].x - mrks[1].x);
      //眉間のY座標：21番Yと22番Yの平均
      ...
      */
      ...
      /*Q8--------------------------------------------------------------------*/
      let kutiUeY = mrks[52].y;
      if (mrks[50].y < mrks[52].y) {
        kutiUeY = mrks[50].y;
      }

      //唇の高さY
      let kutiTakasaY = (mrks[57].y - kutiUeY);
      //唇の幅X
